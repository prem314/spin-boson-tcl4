...
PlotFunc[n_, range_:Full] := Module[{plot, fullPath, yLabel, legendLabels, plotStyles},
  ...
  yLabel = Style[TraditionalForm@AngleBracket[Subscript["\[Sigma]", n - 1]][t], FontSize -> 24, Black];

  (* Construct legend labels using TraditionalForm and Style *)
  legendLabels = {
    Style[Row[{TraditionalForm@AngleBracket[Subscript["\[Sigma]", n - 1]], "(t) (TCL2)"}], FontSize -> Font1],
    Style[Row[{TraditionalForm@AngleBracket[Subscript["\[Sigma]", n - 1]], "(t) (TCL4)"}], FontSize -> Font1]
   };
  ...
  plot = Plot[
    ...
    FrameLabel -> {
      Style["t", FontSize -> 24,  Black], (* X-axis label: Larger, Bold *)
      yLabel                                   (* Y-axis label: Using pre-defined styled label *)
     },
    LabelStyle -> Directive[Black, FontSize -> Font1], (* Default style for labels (like ticks) *)
...
(* --- Legend Enhancements --- *)
    ...
    FrameStyle -> Directive[Black, Thickness[0.002]] (* Make axes lines slightly thicker *)
